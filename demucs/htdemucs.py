...
class HTDemucs(nn.Module):
    ...
    def __init__(
        ...
        self.freq_emb = None
        self.velocity_branch = velocity_branch
        assert wiener_iters == end_iters

        self.encoder = nn.ModuleList()
        self.decoder = nn.ModuleList()
        self.decoder_vel = nn.ModuleList()

        self.tencoder = nn.ModuleList()
        self.tdecoder = nn.ModuleList()
        self.tdecoder_vel = nn.ModuleList()
        ...
        for index in range(depth):
            ...
            if freq:
                ...
                if velocity_branch:
                    self.tdecoder_vel.insert(0, tdec_vel)
            self.decoder.insert(0, dec)
            if velocity_branch:
                self.decoder_vel.insert(0, dec_vel)
    ...
    def forward(self, mix):
        ...
        if self.velocity_branch:
            for idx, decode in enumerate(self.decoder_vel):
                skip = saved_vel.pop(-1)
                x_vel, pre = decode(x_vel, skip, lengths_vel.pop(-1))
                # `pre` contains the output just before final transposed convolution,
                # which is used when the freq. and time branch separate.

                offset = self.depth - len(self.tdecoder_vel)
                if idx >= offset:
                    tdec_vel = self.tdecoder_vel[idx - offset]
                    length_t_vel = lengths_t_vel.pop(-1)
                    if tdec_vel.empty:
                        assert pre.shape[2] == 1, pre.shape
                        pre = pre[:, :, 0]
                        xt_vel, _ = tdec_vel(pre, None, length_t_vel)
                    else:
        ...
        if self.velocity_branch:
            assert len(saved_vel) == 0
            assert len(lengths_t_vel) == 0
            assert len(saved_t_vel) == 0
        ...
        if self.velocity_branch:
            zout_vel = self._mask(z, x_vel)
            if self.use_train_segment:
                if self.training:
                    x_vel = self._ispec(zout_vel, length)
                else:
                    x_vel = self._ispec(zout_vel, training_length)
            else:
                x_vel = self._ispec(zout_vel, length)

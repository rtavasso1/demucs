...
if init_from == 'scratch':
    ...
    if model_name == 'OaF':
        print("Resuming training from a checkpoint is not supported for OnsetsAndFrames")
        model = OnsetsAndFrames()
    else:
...
WINDOW_LENGTH = 2048
melspectrogram = MelSpectrogram(N_MELS, SAMPLE_RATE, WINDOW_LENGTH, HOP_LENGTH, mel_fmin=MEL_FMIN, mel_fmax=MEL_FMAX).to(accelerator.device)
    ...
    for loader, split in zip([train_dataloader, val_dataloader],[ 'train', 'val']):
        ...
        with ctx:
            for batch in loader:
                ...
                mel = melspectrogram(waveform.squeeze()) 
                #print('input shape: ', waveform.shape, onset.shape, velocity.shape, mel.shape)
                ...
                #print('pred shape: ', onset_pred.shape, velocity_pred.shape)
